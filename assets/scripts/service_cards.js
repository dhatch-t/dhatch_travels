const serviceDetails = {
    holiday: {
        ...
        description: "Details about holiday packages, including destinations, deals, and offers.",
    ...
    cab: {
        title: "Cab Services",
        description: "Details on cab services available for airport transfers and city rides.",
...
modalContent.style.width = `${finalWidth}px`;
modalContent.style.height = finalHeight !== 'auto' ? `${finalHeight}px` : 'auto';
modalContent.style.left = `${finalX}px`;
modalContent.style.top = `${finalY}px`;
}, 10);
});
...
const originalCard = document.querySelector(`.service-card[data-service="${serviceType}"]`);
...
if (event.key === 'Escape' || event.key === 'Esc') { // For older browsers 'Esc' might be used
closeModal();
document.body.style.overflow = ''; // Restore page scroll
}
});
